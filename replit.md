...
**DB Tables:** `schedules` (with year/semester), `restaurant`, `courses`, `todos`, `grades` (year, semester, subjectName, credits, grade, category); `users` (includes `college` nullable text column added for OCR-extracted college)
